"""End-to-end daily and minute downsampling pipeline."""
...
from .config import DAYS_PER_CHUNK, METRICS, OUTPUT_DIR, OVERWRITE
...
def _save_minute_tables_for_day(
    tables: dict[str, pd.DataFrame], date: str, out_dir: Path
) -> None:
    for metric in METRICS:
...
def _list_missing_minute_dates(out_dir: Path, dates: Iterable[str]) -> list[str]:
    return [d for d in dates if not _minute_day_complete(out_dir, d)]
...
def run(
    ...
    if overwrite or not _daily_complete(out_dir, len(dates)):
        t0 = time.time()
        logger.info("building daily tables for %d dates", len(dates))
        _save_daily_tables(build_daily_tables(dates), out_dir)
        logger.info("daily complete in %.1fs", time.time() - t0)
    else:
    ...
    for chunk_start in range(0, len(target_dates), days_per_chunk):
        chunk = target_dates[chunk_start:chunk_start + days_per_chunk]
        for date in chunk:
            ts = pd.Timestamp(date)
            prior = close.index[close.index < ts]
            prev_close = close.loc[prior[-1]].to_dict() if len(prior) else None
            tables = build_minute_table_for_date(date, prev_close, stocks)
            _save_minute_tables_for_day(tables, date, out_dir)
            logger.info("minute %s complete", date)
