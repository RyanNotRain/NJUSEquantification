...
- Task4：正式4因子 `adaptive_close`/`adaptive_open` 总收益分别为 19.85%/52.76%，同口径市场超额为 12.00%/40.64%。首次持仓后的区块 Bootstrap 显示，open 几何超额 42.45%的95%区间为6.04%–91.26%、p=0.008；close 区间跨零。最近45日均略跑输市场。
...
- 直接市场超额标签：Ridge/HistGB 测试 Rank IC 为 -0.097/-0.084，最好几何超额 +0.49%，没有超过原始收益 Ridge。
- 原题 `Time/Price/Volume/BSFlag` 四字段映射基线同覆盖超额 -2.40%，说明四项信息不足、25 项存在冗余，19 项目前最均衡。
- 当前共有 49 个自动测试，全项目产物验收为 `passed`。
- 教师版 Markdown 新增“完整结果附录”，集中展示10因子完整评价与分层图、扩展10因子回测、18组Task4参数、Task5强基线/组件消融/幅度预测、10种可交易期限和证据等级；正负结果均保留。
