...
正式文字交付物只保留项目根目录的 `兆信基金量化研究中期报告_最终版.md`。该报告已整理为教师审阅版：第1节逐项对应题目五项要求，Task1–5正文均明确列出“题目要求—本项目作答—正式产物”，扩展研究单独标识；第13节集中收录正文未完全展开的10因子、Task4参数敏感性、Task5模型消融与可交易期限完整结果。`README.md` 负责快速导航，`PROJECT_STATUS.md` 负责验收状态，避免多份报告出现数字和口径不一致。

## 当前正式结论

- Task 1：完成 302 个交易日、300 只股票的日频与分钟频降采样。日频和分钟频均包含 OHLC、成交量、成交笔数、成交额、主买/主卖量额共 11 个字段；OHLC 已使用 `adjfactor.pkl` 复权。
- 数据质量扩展：全量日频与均匀抽取24个分钟交易日的规则违例为0；抽样分钟表均为242×300，15:00收盘与日频收盘最大误差为0。
...
- Task 4 时间折扩展：40/60/90日×Top5/10/20×close/open共18组配置均切成4个不重叠折。正式60日Top10的close有3折正超额，open为2折；18组中14组至少3折为正，但仍不能据此事后改选参数。
...
- Task 5 不确定性扩展：按10个测试交易日整块Bootstrap，Accuracy 95%区间为44.81%–48.25%，仍高于43.68%多数类基线；balanced T+1超额区间为-0.37%–1.13%，经济证据仍跨0。
- 质量控制：49个自动测试全部通过；因子稳健性模块与正式评价统一使用“信号日和实现日均有交易”的样本口径。
...
| 综合扩展 | `scripts/run_comprehensive_extensions.py` | 数据质量、因子多期限、Task4四折、Task5不确定性四组输出 |
