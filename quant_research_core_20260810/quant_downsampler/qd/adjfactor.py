"""Adjustment-factor loading, validated code mapping, and normalized ratios."""
...
def code_mapping(path: str | Path = ADJFACTOR_PATH) -> dict[str, str]:
    """Map six-digit raw codes to the unique supplied full code."""
...
def get_adjust_ratio(data_code: str, date: str) -> float:
    ...
    if timestamp not in series.index:
        raise KeyError(f"missing adjfactor for {full_code} on {timestamp.date()}")
    current = float(series.loc[timestamp])
    if not np.isfinite(current) or current <= 0:
        raise ValueError(f"invalid adjfactor for {full_code} on {timestamp.date()}: {current}")
