...
from .lstm_ensemble import blend_probabilities, two_stage_probabilities
...
def _multiclass_report(true: np.ndarray, probability: np.ndarray) -> dict:
    ...
    precision, recall, f1, support = precision_recall_fscore_support(
        true, probability.argmax(axis=1), labels=[0, 1, 2], zero_division=0
    )
    return {
        "accuracy": raw["accuracy"],
        "macro_precision": raw["precision"],
        "macro_recall": raw["recall"],
        "balanced_accuracy": raw["recall"],
        "macro_f1": raw["f1"],
...
def _write_diagnostics(
    ...
    """Persist report-ready tables and a four-panel evaluation dashboard."""
    import matplotlib
    ...
    true = predictions["true_label"].to_numpy(dtype=int)
    confidence = predictions["confidence"].to_numpy(dtype=float)
    order = np.argsort(-confidence)
    coverage_rows = []
    for requested in np.linspace(0.05, 1.0, 20):
        count = max(1, int(np.ceil(len(order) * requested)))
        selected = order[:count]
        coverage_rows.append({
            "coverage": float(count / len(order)),
            "n": int(count),
            "accuracy": float((predicted[selected] == true[selected]).mean()),
            "minimum_confidence": float(confidence[selected].min()),
        })
    coverage_table = pd.DataFrame(coverage_rows)
    coverage_table.to_csv(target / "confidence_coverage.csv", index=False)

    stock_table = pd.DataFrame([
        {"stock": stock, **values}
        for stock, values in metrics["per_stock"].items()
    ])
    stock_table.to_csv(target / "per_stock_metrics.csv", index=False)
    day_table = pd.DataFrame([
        {"date": date, **values}
        for date, values in metrics["per_day"].items()
    ])
    day_table.to_csv(target / "per_day_metrics.csv", index=False)

    confusion = np.asarray(metrics["confusion_matrix"], dtype=float)
    row_totals = confusion.sum(axis=1, keepdims=True)
    normalized = np.divide(
        confusion, row_totals, out=np.zeros_like(confusion), where=row_totals != 0
    )
    figure, axes = plt.subplots(2, 2, figsize=(12, 9))
    image = axes[0, 0].imshow(normalized, cmap="Blues", vmin=0, vmax=1)
    for row in range(3):
        for column in range(3):
            axes[0, 0].text(
                column, row,
                f"{int(confusion[row, column])}\n{normalized[row, column]:.1%}",
                ha="center", va="center",
                color="white" if normalized[row, column] > 0.55 else "black",
            )
    axes[0, 0].set_xticks(range(3), CLASS_NAMES)
    axes[0, 0].set_yticks(range(3), CLASS_NAMES)
    ...
    axes[0, 1].plot(
        coverage_table["coverage"], coverage_table["accuracy"], marker="o"
    )
    axes[0, 1].axhline(
        metrics["majority_baseline"], color="gray", linestyle="--",
        label="Majority baseline",
    )
    axes[0, 1].set_xlabel("Coverage (highest confidence first)")
    axes[0, 1].set_ylabel("Realized test accuracy")
    axes[0, 1].set_title("Accuracy-coverage trade-off")
    axes[0, 1].grid(alpha=0.3)
    axes[0, 1].legend()

    axes[1, 0].bar(stock_table["stock"], stock_table["accuracy"])
    axes[1, 0].axhline(metrics["majority_baseline"], color="gray", linestyle="--")
    axes[1, 0].tick_params(axis="x", rotation=35)
    axes[1, 0].set_ylim(0, max(0.55, float(stock_table["accuracy"].max()) + 0.05))
    axes[1, 0].set_ylabel("Accuracy")
    axes[1, 0].set_title("Per-stock accuracy")
    axes[1, 0].grid(axis="y", alpha=0.3)

    axes[1, 1].plot(day_table["date"], day_table["accuracy"], marker="o")
    axes[1, 1].axhline(metrics["majority_baseline"], color="gray", linestyle="--")
    axes[1, 1].tick_params(axis="x", rotation=45)
    axes[1, 1].set_ylabel("Accuracy")
    axes[1, 1].set_title("Daily test accuracy")
    axes[1, 1].grid(alpha=0.3)

    figure.tight_layout()
    figure.savefig(target / "evaluation_dashboard.png", dpi=180)
    plt.close(figure)
...
def _write_readme(target: Path, config: dict, metrics: dict) -> None:
    ...
    clean_lines = [
        "# 全窗口下一分钟跌/平/涨 LSTM 三组件融合模型",
        "",
        "本目录由正式训练入口自动生成。组件早停、融合参数与置信度阈值全部由训练集和验证集确定，冻结后才首次读取测试日期。",
        "",
        "## 模型设置",
        "",
        ...
        "- direction：非平盘样本上的跌/涨二分类；基础特征、全局标准化、不含股票 ID。",
        "- movement：平盘/变动二分类；增强特征、逐股票标准化、含股票 ID。",
        "- joint：跌/平/涨直接三分类；增强特征、逐股票标准化、含股票 ID。",
        f"- 验证集融合参数：move_bias={config['move_bias']:.4f}，joint_weight={config['joint_weight']:.4f}",
        "",
        "## 测试结果",
        "",
        f"- 全窗口 Accuracy：{metrics['accuracy']:.4f}",
        f"- Balanced Accuracy：{metrics['balanced_accuracy']:.4f}",
        f"- Macro F1：{metrics['macro_f1']:.4f}",
        f"- 多数类基线：{metrics['majority_baseline']:.4f}",
        f"- balanced 档：accuracy={balanced['test_accuracy']:.4f}，coverage={balanced['test_coverage']:.2%}",
        f"- strict 档：accuracy={strict['test_accuracy']:.4f}，coverage={strict['test_coverage']:.2%}",
        "",
        "## 产物",
        "",
        "- `model.pt`：三个组件权重、结构、标准化参数和冻结的融合配置。",
        "- `test_metrics.json`：全窗口、逐类别、逐股票、逐日及选择性指标。",
        "- `validation_predictions.csv` / `test_predictions.csv`：逐样本标签与各组件概率。",
        "- `training_history.png` / `evaluation_dashboard.png`：训练和评价图。",
        "- `confidence_coverage.csv`：从高置信度到全覆盖的准确率—覆盖率曲线。",
        "",
        "可运行 `python -m scripts.evaluate_lstm_ensemble --model <本目录/model.pt>` 从分钟表严格重载复核。",
        "固定测试日期此前已被研究过程查看，因此结果属于统一口径的样本外诊断，而不是全新盲测。",
    ]
    ...
    lines = [
        ...
        "",
        "可使用 `python -m scripts.evaluate_lstm_ensemble --model <本目录/model.pt>` 从原始分钟表严格重载复核。",
...
def run_full_training(
    *,
    ...
    target = Path(out_dir or (OUTPUT_DIR / "lstm_ensemble"))
    ...
    test_dates = test_metadata["date"].astype(str).to_numpy()
    for date in sorted(np.unique(test_dates)):
    ...
    full_config = {
        "pipeline_version": 2,
        "training_entrypoint": "python -m scripts.run_lstm_ensemble",
        "stock_codes": stocks,
        "channels": list(CHANNELS),
        "splits": ranges,
        "seq_len": seq_len,
    ...
    prediction_table["direction_prob_up"] = direction_test_probability[:, 1]
    prediction_table["movement_prob_move"] = movement_test_probability[:, 1]
    prediction_table["joint_prob_down"] = joint_test_probability[:, 0]
    prediction_table["joint_prob_flat"] = joint_test_probability[:, 1]
    prediction_table["joint_prob_up"] = joint_test_probability[:, 2]
    prediction_table["staged_prob_down"] = staged_test_probability[:, 0]
    prediction_table["staged_prob_flat"] = staged_test_probability[:, 1]
    prediction_table["staged_prob_up"] = staged_test_probability[:, 2]
    ...
    staged_validation_probability = two_stage_probabilities(
        movement_val_probability[:, 1], direction_full_val_probability[:, 1],
        fusion["move_bias"],
    )
    validation_prediction = joint_data["val_metadata"].drop(columns="stock_id").copy()
    validation_prediction["true_label"] = joint_data["val_labels"]
    validation_prediction["predicted_label"] = validation_probability.argmax(axis=1)
    validation_prediction["prob_down"] = validation_probability[:, 0]
    validation_prediction["prob_flat"] = validation_probability[:, 1]
    validation_prediction["prob_up"] = validation_probability[:, 2]
    validation_prediction["direction_prob_up"] = direction_full_val_probability[:, 1]
    validation_prediction["movement_prob_move"] = movement_val_probability[:, 1]
    validation_prediction["joint_prob_down"] = joint_val_probability[:, 0]
    validation_prediction["joint_prob_flat"] = joint_val_probability[:, 1]
    validation_prediction["joint_prob_up"] = joint_val_probability[:, 2]
    validation_prediction["staged_prob_down"] = staged_validation_probability[:, 0]
    validation_prediction["staged_prob_flat"] = staged_validation_probability[:, 1]
    validation_prediction["staged_prob_up"] = staged_validation_probability[:, 2]
    validation_prediction["confidence"] = validation_probability.max(axis=1)
    validation_prediction["selected_balanced"] = (
        validation_prediction["confidence"] >= selective_thresholds["balanced"]
    )
    validation_prediction["selected_strict"] = (
        validation_prediction["confidence"] >= selective_thresholds["strict"]
    )
    validation_prediction.to_csv(target / "validation_predictions.csv", index=False)
    _plot_histories(histories, target)
    _write_diagnostics(target, metrics, prediction_table)
