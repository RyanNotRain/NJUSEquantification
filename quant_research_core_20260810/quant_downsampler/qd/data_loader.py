...
输出: dict[stock_code, DataFrame],每只股票一个 DataFrame,
      含以下列:
        ...
        - minute_bar: 0..241 的展示分钟索引(开盘集合竞价为 -1)
...
_REQUIRED_COLUMNS = tuple(_DTYPES)


def _read_tick_csv(path: Path) -> pd.DataFrame | None:
    """Read one tick file and fail early with an actionable schema error."""
    try:
    ...
    if missing:
        raise ValueError(f"Tick file has missing columns {sorted(missing)}: {path}")
    if df.empty:
        return None
